...
#(one row per mapping unit) containing the TBDeff slope and the auxiliary geological parameters,
#computes Pearson correlations and bootstrap confidence intervals, determines correlation sign,
#and writes a Table_2 CSV and a human_readable Markdown table.
#- Input: the script expects the per‑unit TBDeff slope
#(computed from the four normalized channels) and the auxiliary parameters
#(TiO₂, FeO, Bolometric T anomaly, Regolith T anomaly, Plagioclase, Hydrogen,
#Crustal thickness, Free‑air gravity, OMAT) in a single unit_aggregates.csv.
#- Computation: for each parameter it computes the Pearson correlation with slope,
#estimates 95% bootstrap confidence intervals for the correlation,
#and a two‑sided bootstrap p‑value. The script records the sign (+/−) and
#attaches the geological interpretation strings you provided.
#- Output: table2_summary.csv (numeric results and CIs) and table2_markdown.md
#(manuscript‑ready table). These files correspond to Table 2 in the paper.
#- Reproducibility: N_BOOT and RANDOM_SEED are explicit;
#include these values and the script in your code archive and cite them in Methods (Section 3.6).
